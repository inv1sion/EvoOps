...
func RegisterLocal(ctx context.Context, registry *Registry, repo dataset.Repository) error {
	...
	knowledge, err := utils.InferTool(ToolKnowledge, "Retrieve operating playbooks related to diagnosed signals.",
		func(ctx context.Context, input knowledgeInput) ([]domain.KnowledgeArticle, error) {
			return repo.SearchKnowledge(ctx, input.StoreID, input.Query, input.TopK)
		})
	if err != nil {
		return err
	}
